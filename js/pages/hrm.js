...
async function deleteEmployee(id) {
    if(confirm("Hapus data karyawan ini?")) { await db.employees.delete(id); loadEmployees(); }
}
...
async function deleteCustomer(id) {
    if(confirm("Hapus data pelanggan ini?")) { await db.customers.delete(id); loadCustomers(); }
}

// =====================================
// 3. BUKU PIUTANG (KASBON PELANGGAN)
// =====================================
async function loadPiutang() {
    ...
    try {
        // Tarik data dari tabel Kasir (sales) yang statusnya Piutang (Belum Lunas)
        const piutangData = await db.sales.filter(sale => sale.status === 'Piutang').toArray();
        
        if(piutangData.length === 0) {
            list.innerHTML = `
                <div class="col-span-full text-center py-10 bg-white border border-slate-100 rounded-2xl">
                    <i data-lucide="wallet" class="w-12 h-12 text-slate-300 mx-auto mb-3"></i>
                    <p class="text-slate-500 text-sm font-medium">Belum ada pelanggan yang Kasbon.</p>
                </div>`;
        } else {
            for (let sale of piutangData) {
                // Hitung sisa yang belum dibayar
                const sisaTagihan = sale.total - sale.amount_paid;
                totalPiutang += sisaTagihan;
                
                // Cari nama pelanggan
                const cust = await db.customers.get(sale.customer_id);
                const custName = cust ? cust.name : 'Pelanggan Dihapus';
                
                const dateStr = new Date(sale.date).toLocaleDateString('id-ID', { day: '2-digit', month: 'short', year: 'numeric' });

                list.innerHTML += `
                    <div class="bg-white border-l-4 border-l-orange-500 p-4 rounded-xl shadow-sm relative">
                        <div class="flex justify-between items-start mb-2">
                            <div>
                                <h4 class="font-extrabold text-sm text-slate-800">${custName}</h4>
                                <p class="text-[10px] font-mono text-slate-500">${sale.receipt_no}</p>
                            </div>
                            <span class="bg-slate-100 text-slate-600 text-[9px] font-bold px-2 py-0.5 rounded uppercase">${dateStr}</span>
                        </div>
                        
                        <div class="mt-3 mb-4 bg-slate-50 p-2 rounded-lg text-xs">
                            <div class="flex justify-between text-slate-500 mb-1"><span>Total Belanja:</span> <span>Rp ${sale.total.toLocaleString('id-ID')}</span></div>
                            <div class="flex justify-between text-emerald-600 font-bold border-b border-slate-200 pb-1 mb-1"><span>Telah Dibayar (Cicilan):</span> <span>Rp ${sale.amount_paid.toLocaleString('id-ID')}</span></div>
                            <div class="flex justify-between font-extrabold text-orange-600 mt-1"><span>SISA KASBON:</span> <span>Rp ${sisaTagihan.toLocaleString('id-ID')}</span></div>
                        </div>
                        
                        <div class="flex justify-end">
                            <button onclick="bayarPiutang(${sale.id}, ${sisaTagihan})" class="bg-orange-500 text-white text-xs font-bold px-4 py-2.5 rounded-xl hover:bg-orange-600 transition shadow-sm flex items-center">
                                <i data-lucide="hand-coins" class="w-3 h-3 mr-1.5"></i> Terima Pembayaran
                            </button>
                        </div>
                    </div>
                `;
        ...
        document.getElementById('total-piutang').textContent = `Rp ${totalPiutang.toLocaleString('id-ID')}`;
        lucide.createIcons();
    } catch (e) { console.error("Error muat Piutang:", e); }
}

async function bayarPiutang(saleId, sisaTagihan) {
    let inputCicilan = prompt(`Masukkan nominal pembayaran yang diterima dari Pelanggan (Sisa Kasbon: Rp ${sisaTagihan.toLocaleString('id-ID')}):`);
    
    if (inputCicilan === null || inputCicilan === "") return;
    
    let nominalMasuk = parseInt(inputCicilan);
    if (isNaN(nominalMasuk) || nominalMasuk <= 0) return alert("Nominal tidak valid!");

    try {
        const sale = await db.sales.get(saleId);
        let newAmountPaid = sale.amount_paid + nominalMasuk;
        let newStatus = 'Piutang';

        if (newAmountPaid >= sale.total) {
            newAmountPaid = sale.total;
            newStatus = 'Lunas';
            alert("Mantap! Pelanggan telah melunasi tagihannya.");
        } else {
            alert(`Cicilan sebesar Rp ${nominalMasuk.toLocaleString('id-ID')} berhasil dicatat. Sisa kasbon: Rp ${(sale.total - newAmountPaid).toLocaleString('id-ID')}`);
        }

        // Update database Kasir
        await db.sales.update(saleId, { 
            status: newStatus, 
            amount_paid: newAmountPaid 
        });
        
        loadPiutang(); // Refresh Buku Piutang
    } catch (error) {
        console.error("Gagal menerima pembayaran:", error);
    }
}
...
document.addEventListener('DOMContentLoaded', () => {
    if (typeof db !== 'undefined') {
        loadEmployees();
        loadCustomers();
        loadPiutang(); // Panggil fungsi muat piutang saat halaman dibuka
        lucide.createIcons();
    }
});